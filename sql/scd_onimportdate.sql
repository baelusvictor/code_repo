...
from(
    values
        ...
        ('A','JL', date(cast('2016-03-21 15:19:34.0' as timestamp))),
        ('A','JL', date(cast('2016-03-22 15:19:34.0' as timestamp))),
        ...
        ('Z','JL', date(cast('2016-03-25 15:19:34.0' as timestamp))),
...
select 
    *,
    min(tijd) over (partition by partitie, naam) as van,
    max(tijd) over (partition by partitie, naam) as tot
from _final
order by tijd
),
...
select * from _finalfinal -- _yesrange